package seahorse.internal.business.openapi.katavuccolopenapi.common;
/**
 * 
 */

/**
 * @author SMJE
 *
 */
public class KatavuccolOpenApiErrorCode {

	public static final String CreateProfileApiMsgEntityIsEmpty = "KatavuccolOpenApi.API.{0}.{1}.CreateProfileApiMsgEntity.Empty";
	public static final String InternalError = "KatavuccolOpenApi.API.{0}.{1}.InternalError";
	public static final String ErrorMakingProfileServiceExternalCall = "KatavuccolOpenApi.API.{0}.{1}.ErrorMakingProfileServiceExternalCall";

}
